...
class GenderClassification:
    def __init__(self, modelPath=None, device='cpu'):
        self.device = device
        self.model = bilinear_resnet.BCNN(2, pretrained=False).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            for i, (images, labels) in enumerate(trainData):
                ...
                with open('train.txt', 'a') as f:
                    f.write(f'Epoch {epoch + 1}: Iter {i + 1}, Train loss {loss}\n')
            ...
            self.model.train()
            with open('train.txt','a') as f:
                f.write(f'Epoch [{epoch + 1}/{num_epochs}]: train Loss: {loss} Train_Acc: {train_acc}  test Loss: {test_loss} Test_Acc: {test_acc}\n')
